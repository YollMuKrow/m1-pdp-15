...
class TestCSV(unittest.TestCase):
    ...
    def test_adding_column_selfplay(self):
        o = csvLib.CsvOutput("random", "random", "test_adding_column_selfplay")
        o.check_ia_in_fieldnames("test", "test")
        with open(o.get_output_file_name(), 'r') as file:
            csv_reader = csv.DictReader(file)
            header = list(csv_reader.fieldnames)
            self.assertEqual(header, ['random', 'test'])
        clean_test_file(o.get_output_file_name())

    def test_adding_column(self):
        o = csvLib.CsvOutput("random", "cma", "test_adding_column")
        o.check_ia_in_fieldnames("test", "random")
        with open(o.get_output_file_name(), 'r') as file:
            csv_reader = csv.DictReader(file)
            header = list(csv_reader.fieldnames)
            self.assertEqual(header, ['random', 'cma', 'test'])
        clean_test_file(o.get_output_file_name())

    def test_no_adding_column_on_existing_file(self):
        o = csvLib.CsvOutput("random", "cma", "test")
