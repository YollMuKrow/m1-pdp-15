...
games['robocup_adult_size'] = Game(env_name='AdultSizeEnv',
  input_size=20,
  output_size=7,
  time_factor=0,
...
games['robocup_kid_size'] = Game(env_name='KidSizeEnv',
  input_size=20,
  output_size=7,
  time_factor=0,
  layers=[20, 20], # hidden size of 20x20 neurons
...
def makeRoboCupAdultSizePolicy(filename):
  model = Model(games['robocup_adult_size'])
  model.load_model(filename)
  return model

def makeRoboCupKidSizePolicy(filename):
  model = Model(games['robocup_kid_size'])
  model.load_model(filename)
  return model
...
class Model:
  ...
  def load_model(self, filename):
    ...
    self.data = data
    model_params = np.array(data[0]) # assuming other stuff is in spec
